...
“Codex 回复”，最后用一条紧凑底栏展示精确 task 名称和 Codex 运行时；从飞书发起的回合只显示
回复和底栏，避免重复原话题里已有的输入。飞书会话列表预览本地输入或回复的第一条有效内容，
不复述底栏元信息。卡片以颜色区分普通答复、里程碑、待拍板和风险，没有按钮或回调，不改变
既有入站和授权边界。
...
同时包含“你的输入”和“Codex 回复”的 Card 2.0，两个区块来自同一个 `turn_id`。
