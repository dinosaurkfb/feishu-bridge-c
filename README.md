...
每一笔的执行 / 重放 / 维护恢复都在同一把逐 key 事务锁（`<key>.control.lock`，mkdir 原子，owner.json 记 pid/时间/token）里判定与动手，隔离改名也在锁内；
锁内状态对所有调用者权威（晚到的首次调用者撞见已闭合的记录只重出回执，不再执行）；释放前核 token，不是自己的实例不删，删不掉也不吞 —— 以 lockUncleared 进入结果（维护入口退出 1、回执写明）；
