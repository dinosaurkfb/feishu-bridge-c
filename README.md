...
直接输入的本地回合，也会把“你的输入 + Agent 回复”合成一张卡片留下完整记录。
...
Agent 回复居中，精确任务名和 Codex/Claude 运行时保留在底部紧凑栏；飞书会话列表则直接预览
本地输入或 Agent 回复的第一条有效内容，而不是重复任务元信息。蓝、绿、橙、红分别表示答复、
里程碑、待拍板和风险。卡片只做结构化展示，不含按钮或回调。发布失败时答复会留在本机 outbox，
不会被假装成已经送达。
...
发起的回合只显示回复；长内容默认折叠，完整事件仍保存在本机 outbox/run 记录中。
