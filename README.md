...
“进展怎么样”。Codex 的最终答复、进展与风险会用 Card 2.0 卡片呈现，卡片标题保留精确长期
任务名称，并用蓝、绿、橙、红区分答复、里程碑、待拍板和风险；当前卡片只做结构化展示，不含
按钮或回调。发布失败时答复会留在本机 outbox，不会被假装成已经送达。
...
7. 同一个第三方智能体把答复发布回原话题；Codex 结果以只读 Card 2.0 卡片呈现
```
...
Codex 卡片只用于绑定完成后的结果回复。首次建话题的根消息及接入状态继续使用文本，因为首次
Aily 绑定要从根消息引用中读取六位短码；卡片不会改变话题 ID、session 映射、入站规则、outbox
或自动发布资格。长答复默认折叠显示，完整事件仍保存在本机 outbox/run 记录中。
