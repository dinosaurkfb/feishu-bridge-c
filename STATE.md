...
3. 消息新不新
4. 全机同时**只有一份**待绑定
5. 待绑定没超过 **24 小时**（`bound_at` 都读不出来的当已过期 —— fail-closed）
...
意味着「认领哪个待绑定」可以从「全机只有一份 + 24 小时窗口」升级成**确定性匹配**。
**但只有一次观测，先不改** —— 这个项目在「单次观测下结论」上栽过四次。
至少看到第二次（下一个项目接入时）再动。绑定码从第一天就写进了根消息，
就是为了将来升级不用迁移已建好的话题。

## 没做完的

1. **确定性认领还没做**（见上）。现在多个项目同时待绑定会拒绝，不会认错 ——
   安全但不方便。
2. **事件筛选仍然很粗。**噪音水平还是没验证过 —— 需要跑几天真实使用。
3. **「没记进展就不许收工」没做。**Stop 钩子的 `decision: "block"` 仍未实测。
