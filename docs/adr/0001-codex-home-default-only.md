...
`$HOME/.codex`，账簿仍未脱离运输现场（Codex 一轮 P1-2；已实测两者差异）。

**不提供任何切换账簿的命令行参数**：真入口会把 argv 原样转交 handler，一个"只有测试才传"的参数在生产同样可达，
等于把本 ADR 排除掉的支持面又开了回来（Codex 二轮 P1）。测试改走 import 形参或测试专用的启动注入，
产品代码里不存在对应分支。
