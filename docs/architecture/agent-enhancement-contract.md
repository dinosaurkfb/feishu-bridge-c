...
- 幂等与上界（`chat-ledger.mjs`，机器级）：key = sha256(chain \0 message_id \0 session_id)；`<key>.chat/`（mkdir 原子）+ claim.json + outcome.json（answered 记回答全文 / failed 记受控原因）。重放：answered → 按记录重出（尾行标"重放"），failed / stale（pid 死且无终态）→ 明说不再答、请发新消息，running → 明说还在答；都不再起模型。并发：正在答（claim 在、终态缺席、pid 活、未超预算 + 30 秒宽限）全局 ≤ 2、每发送者 ≤ 1，**在取 claim 之前判**，被拒的那条重放时还能再试。回执写在机器级 unrouted 回执目录（`chat-*`），不出 locator。
