...
- 幂等与上界（`chat-ledger.mjs`，机器级）：key = sha256(chain \0 message_id \0 session_id)；一条 chat = **一个文件** `<key>.chat.json`（claim 与终态同一份 JSON：state = running / answered / failed；answered 记回答全文，failed 记受控 reason 与按 reason 封闭的附加字段）。不用目录：目录会被"换出再换回"绕过任何事后核对；单文件的读写都落在同一个已打开的文件对象上（`O_NOFOLLOW` 打开 → 同一 fd fstat 确认普通文件 → 从这个 fd 读；claim 先把完整记录写进 scratch 子目录 `tmp/` 里唯一命名的临时文件（`<key>.<pid>.<time>.<uuid>`，`O_EXCL`）并 fsync、再 `link` 到正式路径发布 —— link 从不覆盖，EEXIST 即重放，正式路径上没有"空文件"窗口；终态用临时文件 + rename 覆盖，rename 替换的是目录项本身，路径被换成符号链接时替换掉的是链接、不是它指向的东西；清理只删自己的临时文件：删前按 dev/ino 核对、删后用仍打开的 fd 看 nlink 是否真少了一，否则如实报 tmpResidue）。建 claim 与记终态都在同一把账本锁（`admission.lock`，symlink 锁协议）内，且提交动作（link / rename）走 `commitWhileHeld`（在与陈旧回收互斥的 reap 段里核对主锁仍是我这一实例才提交；事务停顿超过 5 分钟被合法回收后返回 lock_lost、不覆盖）：记终态在锁内重读、必须仍是 running（否则 already_final），"终态只记一次"由锁 + fencing 保证。scratch 里的临时文件不参与准入盘点（残留不会让账本"说不清"），只由 doctor 盘：年轻的是进位中，超过 60 秒仍在的是残骸（可直接删），名字 / 类型不对的是说不清；顶层任何非记录名字都是说不清。重放：answered → 按记录重出（尾行标"重放"），failed / stale（pid 死且无终态）→ 明说不再答、请发新消息，running → 明说还在答；都不再起模型。并发：正在答（claim 在、终态缺席、pid 活、未超预算 + 30 秒宽限）全局 ≤ 2、每发送者 ≤ 1，**在取 claim 之前判**，被拒的那条重放时还能再试。回执写在机器级 unrouted 回执目录（`chat-*`），不出 locator。
