...
每个 run 必须携带不可变的 `origin_channel_generation_id`。即使话题在运行中轮转，结果仍发布到该次
请求的来源话题。
...
| Local Target | `local_target_id` | runtime、opaque locator、domain、display title | runtime adapter |
| Channel Binding | `binding_id` | subscription、local target、policy、status | 控制面 |
...
| Run | `run_id` | event、binding generation、target、adapter run、terminal state | 审计面 |
| Outbox Item | `outbox_id` | run、origin、payload、publish state | 审计面 |
...
原始信封和 canonical event 的敏感字段不得输出到普通用户回复。
...
3. 按可信 `session/topic/chat/agent` 和路由表选择唯一 handler/domain；
...
schema validate
  ...
  -> freshness
  -> exact channel generation
  -> atomic claim
  -> policy handler
```

任一步失败都不得进入下一步。claim 键 SHOULD 至少包含 `event_id + route_id`；同一事件在重试时必须
得到幂等结果，不得重复启动目标运行。
...
- `rotating`：旧代际停止接收新事件，新代际完成创建与登记后原子切换；
...
1. 获取 binding 生命周期锁；
2. 为新 generation 预分配 ID；
3. 创建新根话题并登记待认领状态；
4. 完成 session 认领；
5. 原子把新 generation 设为 active、旧 generation 设为 read-only；
6. 释放锁并记录审计事件。

中途失败时旧 generation 继续保持 active，不能产生“两个都收”或“两个都不收”的中间态。
...
- 只发布用户可见内容；
- 默认回复到 `origin_channel_generation_id`。
...
- 可以把消息投递给项目内活着的精确交互会话；
- 没有活动现场时，可以精确 resume/continue 已登记会话；
- 不得把“同一目录下最近会话”当成精确会话绑定的替代。
...
- 每条 outbox 项记录其来源 generation，轮转不改写已有目标。
...
- canonical event schema 与不可信正文隔离；
...
1. 先引入 versioned schema 和只读转换器，不改写现有 registry；
2. 用 shadow comparison 同时计算旧路由与新路由，结果不一致时只记录、不投第二次；
...
| `feat/control-plane-schema` | schema、只读迁移、显式 intent | 数据面切流 |
| `feat/unified-inbound-dispatcher` | dispatcher 与 canonical event | 新交互模式 |
