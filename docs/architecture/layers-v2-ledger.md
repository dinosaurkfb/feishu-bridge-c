...
```
binding_target =
  | { runtime:"claude", project_root:"<abs>", claude_session_id:"<uuid>" | null }
...
| **rebind_session_alias**（W2 认领现场，Frank 拍板）| 目标 live ∧ **当前族仅 B3（active+current）** ∧ expectedOldSessionId=当前 aliases.session_id（CAS）∧ newSessionId 走 AILY_SESSION_SHAPE ∧ 无 live 记录占用该 newSessionId（alias_occupied fail-closed）∧ authorizedBy 受验。**P1-5：B3′ 只读 dormant→target_not_active；B4/其它 非 current→target_not_current（fail-closed，不误绑历史代际）** | **只**改 aliases.session_id 到 newSessionId + updated_at + origin_operation_id=本笔；**不动** binding_target / binding_proof / family / lineage（binding_target 的 retarget 归 Phase 2 配对写方，**当前无消费方、预留**——见 m1a-reconciliation §5.1）|
...
（`initialize_shadow`/`authority_cutover` 属维护层入口：**裸调用恒拒**；受验维护 capability 路径
（R69）已武装，判据只在 verifyCutoverPlan 一处；
