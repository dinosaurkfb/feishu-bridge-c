...
> **状态：第二步定稿（Codex 四轮评审放行，无 P1/P2）。§10 拍板项 Frank 已「按
> 推荐」全数落定（2026-09-04）；deliver 上线授权与写飞书文档同步两项守红线另办。**
...
**封闭判定优先级（P1-3；bind-only 已拍板，§10-1）**——一条入站
消息的处理顺序固定为：
...
| `/feishu-bind`（= create 建桥话题） | 不适用：无绑定飞书话题没有本机项目目标可供现行 bind 建题所依附 → 接入指引（去终端 bind，或用 attach） | 冲突语义（§10-5 已定：拒绝） |
| `/feishu-attach <目标>`（= bind 当前话题） | **适用**（认领当前话题到指定本机项目） | 冲突语义（§10-5 已定：拒绝） |
...
| `/feishu-attach <序号\|名>` | model_control | R3 | 是（正文恰为，封闭） | owner | unbound：秒回执→attach 事务→deliver 回报；bound：拒绝（§10-5） |
...
## 10. 拍板项（Frank 2026-09-04「按推荐」全数落定）

1. 配对复合消息 = **bind-only**（§4）：本条只做 R3 配对、正文不执行、回执要求
   重发。§4 措辞已从「待拍板」转为定案。
2. `/feishu-projects` 清单范围 = **只列已装桥项目 + 活跃工作线**（不列本机全部）；
   与 feishu-initiated-binding.md G1 并轨。
3. operator 的 R0-topic 暴露边界 = **按 §7**：只看投影名+策略，不看本机项目身份。
4. 命令名 = **沿用** `/feishu-projects`、`/feishu-attach`。
5. 同项目多话题 attach 冲突语义 = **先拒绝**（§第一步 §6-2；bound 上下文的 attach
   一律拒，回执说明已绑定）。

第二步定稿，与第一步一并作为 M1 需求输入。第三步（绑定唯一性细化、账本存储与锁）
继续推导。

**未由本次「按推荐」放行的两项（红线，需封闭对象的逐次授权，不能笼统代替）**：
- deliver ① 类自动写入的**上线授权**：Frank 已确认设计方向（M1 用 deliver 做
  attach 回报），但真正上线仍走 `--apply` 安装闸（指向具体 PR+HEAD），届时单独确认；
  现在无 deliver 代码在跑，方向获批不产生任何即时写入。
- **写飞书文档同步**：属「写飞书」，需 Frank 指定封闭目标（新建一篇 / 追加到现有
  《四层》飞书文档并给链接）后逐次执行，不由笼统「按推荐」触发。

## 11. 跨会话转发凭证与接收方核验（PK3-A1）

关于跨会话投递（SendMessage）下目标会话核验 owner 身份与正文防篡改机制，参见：
- `docs/architecture/relay-credential-verification.md`
