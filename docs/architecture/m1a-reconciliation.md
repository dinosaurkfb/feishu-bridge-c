...
| bind/认领（claim→绑定：引用码、@ 配对） | create_a1 → activate（固定顺序）。**W1**：认领在 chat/sender/body(码)/thread_root 四维真实匹配 → f4={matched_om:被引根消息 om, matched_fields:标准四项}（G15 零扩展）、authorized_by=event.sender_id。**W2：再认领（B3 已 active 换会话）= rebind_session_alias**（Phase 1，认领现场：**只**改 aliases.session_id 到受验新 Aily 会话 locator，不动 binding_target/proof/family/lineage），不走 activate | 同一 claim：ext=claim key；create_a1 entity=session locator、activate entity=B1 topic_agent_id、rebind_session_alias entity=B3 topic_agent_id——entity 相异故 key 确定性派生且不同 |
