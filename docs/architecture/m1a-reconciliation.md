...
| `facts` 五元组 | §2 完整给值 |
| `binding_target` | §1 严格采集受验值（Claude 允许 claude_session_id: null 为项目级目标；由持有真实本地 Claude UUID 的显式 retarget 补；owner_select 的 Aily session 不能填这个字段） |
...
| bind/认领（claim→绑定：引用码、@ 配对） | create_a1 → activate（固定顺序）。**W1**：认领在 chat/sender/body(码)/thread_root 四维真实匹配 → f4=判别联合（token 认领 → `binding_token_v1`，matched_fields=标准四项 + `pending_token_state:"present"`；无码 owner-root 认领 → `owner_root_no_token_v1`，matched_fields=三维 `[chat_id,sender,thread_root]` + `pending_token_state:"absent"`，**无 body/码 维**）、matched_om=被引根消息 om、authorized_by=event.sender_id。裁定 d：不放松 F4、不伪造——无码时降成三维 owner-root 证明，不把未验 body 维塞进四项。**W2：再认领（B3 已 active 换会话）= rebind_session_alias**（Phase 1，认领现场：**只**改 aliases.session_id 到受验新 Aily 会话 locator，不动 binding_target/proof/family/lineage），不走 activate | 同一 claim：ext=claim key；create_a1 entity=session locator、activate entity=B1 topic_agent_id、rebind_session_alias entity=B3 topic_agent_id——entity 相异故 key 确定性派生且不同 |
| 本地配对写方（§5.1，**W2 Phase 2**：会话配对实际落库时同步 binding_target 的 claude_session_id）——**预留（当前无消费方）** | **预留**。Codex 裁定协议（Frank 2026-09-xx 拍板并核 bind-session / feishu-rotate / live-session delivery pin）：现行 legacy **不存在就地改既有绑定 claude_session_id 的写方**——bind-session 是新建（新 lineage）、rotate 是新代际创建时带 target、delivery pin 明文「只是偏好不是权威」（写 `.runtime-data/inbound/delivery-session.json` 独立文件，**刻意不复用** claude_session_id）。故 W2 认领后两侧都保持旧 target，双射成立，**Phase 2 不需要 intent 机制**（没有消费方的持久 intent 是死状态）。**将来**引入任何就地改 target 的写方，必须按下面协议接线 + intent 机制一并实现，否则是 cutover blocker | 若将来有写方：W2 驱动=claim key 作持久 intent、非 W2 驱动=需 owner 授权独立 operation id、锁序 outer→legacy→ledger |
