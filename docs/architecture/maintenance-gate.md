...
> **启动源按平台（PK3-L1 / PK3-L3）**：darwin 用 launchd（LaunchAgent plist），linux 用 systemd `--user`
> （`~/.config/systemd/user/feishu-bridge-cc-drain.{service,timer}`，`OnUnitActiveSec=30min`）；
> 其它平台无桥拥有的 timer，按 absent 处理。

| 定时器（按平台：darwin launchd / linux systemd --user） | `launchctl list <label>` / `systemctl --user` + plist/unit | 状态 ∈ {loaded, installed_not_loaded, absent}（下文统称**原始三态**）且配置字节与投影相等；`loaded_other` / `orphan` / `stale` / `partial_unit` / `running_unmanaged` / `transitional` / `plist_unreadable` / `unverifiable` → 不受验（预检拒了，journal 只会记到原始三态） |
