...
| C1b 影子 chat 证据接 env | S | 影子 `evidence.chat_id` 改用 `AILY_CLI_CHANNEL_CHAT_ID`（与 legacy 同源）—— **已落地（PR #211）**：真机样本 S1（第二群 @M5Claude）上 legacy 判 chat_mismatch 拒、影子却 fail-open accepted，根因就是两条路用了不同的证据源；取不到 env 时仍为 null + `scope_unverified=["chat_id"]`，权威面零改动 |
