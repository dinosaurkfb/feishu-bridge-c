...
| 对话模式 | Dialogue v1 已由 PR #11 合并 `main` 并完成 Codex 真实 3 回合验收；Slice A foundation 已形成公共 schema、纯函数 planner、离线 simulator 与双端本地回归候选，但未接 adapter | 评审/合并 Slice A；多订阅路由必须等自动轮转 v1、可信 chat scope 和授权快照同步完成真实验收后再灰度切流，最后才开启固定串行 Agent Relay |
