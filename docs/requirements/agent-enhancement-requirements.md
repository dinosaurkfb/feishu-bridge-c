...
| 项目—群订阅 | Subscription v1、首次认领消费者和 claim 纵切已由 PR #5 合并；尚未开放多订阅写入口。**FR-2.5 的同步计划器已于 2026-08-24 实现**（`subscription-sync.mjs`：算出订阅撤销/暂停/改范围各影响哪些 binding，以及各该 resnapshot / suspend / migrate；多条订阅都能接住同一条 binding 时按 FR-2.6 同一理由拒绝歧义）。**落盘那一半还没有** —— 把计划真正执行到授权快照与 binding 状态上。写入口要等它 | 开放受控多订阅管理，不改变现有单订阅默认行为 |
