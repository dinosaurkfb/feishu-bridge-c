...
| 话题轮转 | Topic Generation 生命周期与显式 rotate 命令已由 PR #8 合并并安装；Codex 第 2 代话题的创建、真实 mention 认领与 binding 切换已完成首次真实验收；自动轮转 v1 已合并 `main`，默认阈值 30。**Codex 曾在临时阈值 5 下完成一次自动轮转的 happy-path 真实验证**（阈值随后恢复 30；该临时构建未推送远端，只创建代际时把 5 写进了当代运行时状态，新代际按代码默认取 30）—— 但**失败重试、不重复轮转与双 runtime 覆盖仍待验收** | 补齐失败重试、不重复轮转与双 runtime 的真实验收，再评估多订阅 |
...
| 发布开关覆盖不全 | 有缺口 | `auto_publish_on_completion` 只被 `inbound.mjs` 与 `watch-and-publish.mjs` 读取；**兜底排空 `drain-outbox.mjs` 不读它**，于是关掉这个开关并不能让进展留在本地。status 已按实际行为措辞（「每轮完成时不自动发布（兜底排空仍会发出）」），但开关本身该不该覆盖兜底路径是一次**行为变更**，要单独决定：让自动调用方都遵守它，还是把它明确定义为只管每轮完成时那一条 |
