...
| 端点自检（FR-1.4） | 2026-08-24 实现 | 区分 bridge 未安装 / adapter 不可用 / daemon 离线 / 身份不匹配，四种的下一步动作不同，混成一句「不可用」等于没说。**只读、限时、不修不启**；三态 pass/fail/unknown，汇总 ready/blocked/incomplete —— **unknown 既不算 fail 也不算 pass**。已并入 `$feishu-status` 第 1 层；不跑自检时那行仍显示「未自检」 |
