...
// PK3-W232-fix2：同一类无条件承诺（"本机输入与每轮回答会合成卡片发到新话题"）在会话级接入这里也要收窄 ——
// 新话题以后落到哪条路由同样不是此刻能定的。
console.log("\n这条会话已单独接入：它的新话题不再走项目那个话题。");
console.log("回复方式取决于新话题的路由 —— 本链自己绑定会话时，进展与每一轮回答会以卡片发到新话题。");
