...
import { ELIGIBILITY_BUDGET_DEFAULT_MS, eligibilityBudgetMs } from "../eligibility-recovery.mjs";
...
// claim key 的形状从 claim.mjs 取 —— 文件名就是 key，所以这一条同时封住了文件名。
 ...
 */
export function recoverEligibilityPending({ claimsDir, outboxDir, publishLockDir, threadId, runsDir }) {
  // **说不清去哪验 run 凭据就一条都不许恢复。**恢复标记只记"资格提升待重试"，
  // 它的 run_state=completed 是自报，不能代替终局证据 —— 两个制品各自验，AND 不是 OR。
  if (typeof runsDir !== "string" || !runsDir) {
    return { ok: false, reason: "runs_dir_required", recovered: [], pending: [], unusable: [] };
  }
  ...
  for (const item of listed.items) {
    ...
    // 授权之前先验这一条 claim 的 run 复合凭据。验不过留着标记、说清原因 ——
    // 不猜，也不因为标记自己说 completed 就放行。
    const credential = verifyCodexRunCredential({
      runsDir, claimKey: item.key, expectedThreadId: threadId });
    if (credential.state !== "completed") {
      pending.push({ ...item, reason: "run_credential_unverified",
        why: credential.state === "running" ? "退出回执缺席"
...
export function settleEligibilityPending({
  claimsDir, outboxDir, publishLockDir, threadId, runsDir,
  ...
  const { last, attempts } = retryUntil({
    ...
    attempt: () => recoverEligibilityPending({ claimsDir, outboxDir, publishLockDir, threadId, runsDir }),
...
export function settleOwnEligibility({
  claimsDir, outboxDir, publishLockDir, threadId, claimKey, runsDir,
  budgetMs = ELIGIBILITY_BUDGET_DEFAULT_MS, waitMs = 1500,
  now = () => Date.now(), wait = waitSync,
}) {
  const once = () => {
    const sweep = recoverEligibilityPending({ claimsDir, outboxDir, publishLockDir, threadId, runsDir });
    const outcome = eligibilityOutcomeFor(sweep, claimKey);
    if (outcome.reason !== "marker_missing") return outcome;
    // 复核也是授权 —— 同样先验 run 复合凭据。
    const credential = verifyCodexRunCredential({ runsDir, claimKey, expectedThreadId: threadId });
    if (credential.state !== "completed") {
      return { ok: false, reason: "run_credential_unverified",
        why: credential.state === "running" ? "退出回执缺席"
          : (credential.reason ?? "说不清") + (credential.why ? "：" + credential.why : "") };
    }
