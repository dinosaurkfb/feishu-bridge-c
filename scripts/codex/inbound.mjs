...
import { evaluateChatGates, CHAT_FALLBACK_REASONS, OFF_TEMPLATE_HINT, PENDING_CLAIM_HINT, isPrivateChatTurn } from "../inbound-route.mjs";
...
if (!routed.ok) {
  ...
  if (!promotion.ok) {
    ...
    const reasonText = (REASON_TEXT[promotion.reason] ?? promotion.reason) +
      ...
      (promotion.pending_claim_hint ? PENDING_CLAIM_HINT : "");
