...
const rejectExpect = { logicalTaskKey: task.logical_task_key, codexThreadId: task.codex_thread_id };
...
const rejectControl = (replay) => {
  const tx = runRejectTransaction({ claimsDir: paths.claims, key: claim.key, projection: rejectedProjection, replay, expect: rejectExpect });
...
if (!claim.ok && claim.reason === "duplicate" && rejectedProjection) {
  ...
  const original = readClaimState({ claimsDir: paths.claims, key: claim.key, expect: rejectExpect });
