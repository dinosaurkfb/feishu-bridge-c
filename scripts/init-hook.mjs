...
export function composeAsk({ cwd, bridgeRoot, chatName }) {
  return [
    "[飞书桥] 这个目录还没有接飞书话题。",
    "",
    "**等你把 CLAUDE.md 写完之后**（不是现在），问 Frank 一句：",
    "要不要在飞书群「" + (chatName ?? "（机器级配置里的那个群）") + "」里给这个项目建一个根话题？**默认「是」。**",
    "",
    "- 答是 → 先跑一次**不带 `--apply`** 的看文案，再跑带 `--apply` 的那条：",
    "  `node " + bridgeRoot + "/scripts/bind-project.mjs --project " + cwd + " --apply`",
    "- 答否 → 什么都不做，这一轮里也不要再问第二次。",
    "",
    "**跑不了就把命令交给他，不要自己还原文案。**这条命令可能被权限或 auto 模式拦下 ——",
    "那不是让你去读 bind-project.mjs 把根消息「逐字还原」一遍的信号。还原出来的东西看着",
    "像脚本的输出，其实是你算的，一旦和真正会发出去的差一个字，他就是照着一份假预览点的头。",
    "拦下了就直说，让他自己敲：`! node " + bridgeRoot + "/scripts/bind-project.mjs --project " + cwd + "`",
    "",
