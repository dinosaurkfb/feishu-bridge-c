 ...
 *
 * 群名三级（评审 #114 二轮 P2）：**条目自带的 chat_name**（控制面条目登记时录入的）>
 * **模板匹配**（调用方从链路模板传进来的 groupName —— 模板里本来就有 chat_name，绑定命令一直在打印
 * 它）> **不可用**。订阅投影不再"自己只有 chat_id"：控制面条目能带 chat_name；取不到名字时显示
 * "不可用"，**不拿 ID 顶替**。
 */
