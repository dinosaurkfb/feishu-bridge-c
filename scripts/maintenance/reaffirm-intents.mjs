...
export function consumeReaffirmIntent({ endpointId, reaffirmHandle, sender, chatId, selectionMessageId, now = undefined, clock = () => Date.now(), env = process.env } = {}) {
  ...
  return withIntentsLock(d.dir, () => {
    ...
    const nowMs = Number.isFinite(now) ? now : clock(); // intent 锁内采样
    if (Date.parse(entry.expires_at) <= nowMs) return { ok: false, reason: "reaffirm_intent_expired", why: "过期不清（等签发侧同锁受验清理）" };
    ...
    const res = ownerSelectReaffirm({
      ...
      selectionMessageId, now: nowMs, clock, env, _inject: undefined,
    });
