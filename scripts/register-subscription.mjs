...
export function describeStoreWrite(r, file) {
  ...
  // FR-2.6 单 4：store 写成 + 审计丢了是「成功但要人知道」，不是静默成功 —— 这块必须上屏 + 非零。
  if (r && r.auditUnwritten) {
    lines.push("注意：审计记录没写上（" + r.auditUnwritten + "）；store 已写入，但这次变更没留下可对账的审计，请人工核对 " + file + ".audit.jsonl");
    exitCode = 1;
  }
