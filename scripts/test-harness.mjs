 ...
 * 没挑中东西"——那是表的错，不是守卫的功劳）。
 *
 * 汇总与退出码在 printSummary 一处收口：失败 > 0 → process.exitCode = 1，并在
 * 打印后断言两者一致。**验收除了 grep 汇总行，必须看退出码** —— 汇总行是打印
 * 出来的，退出码才是机器判据。
 */
...
export function installUnhandledRejectionGuard() {
  process.on("unhandledRejection", (reason) => {
    console.error("\n✗ 未处理的 rejection——多半是 async 用例（注册器不 await，断言不会计入）：\n    " +
...
export function createTestHarness({ onFail = () => {} } = {}) {
  ...
  function printSummary({ suiteLabel = "", printFailures = false } = {}) {
    console.log((suiteLabel === "" ? "\n" : suiteLabel + " ") +
      "通过 " + passed + " / 失败 " + failed + (suiteLabel === "" ? "\n" : ""));
    if (TEST_FILTER.length > 0) {
      console.log("TEST_FILTER 命中 " + executed + " / 总 " + registered
        + "（子串：" + TEST_FILTER.join(" | ") + "）—— 这不是全量，不许当全量绿");
    }
    // **汇总与退出码一致**（R59）：失败 > 0 → 退出码 1。这行断言钉住两者不得各说各话 ——
    // 2026-09-10 验收 R57d 时四方都只 grep 汇总行，一个 async 用例的 unhandled rejection
    // 只有退出码能看出来。改汇总逻辑时若把这条漂掉，下一轮验收就会只看打印行。
    process.exitCode = failed > 0 ? 1 : process.exitCode;
    if (!(failed === 0 || process.exitCode === 1)) {
      console.error("✗ 汇总与退出码不一致：失败 " + failed + " 但 process.exitCode=" + process.exitCode);
      process.exit(1);
    }
    if (failed > 0) {
