...
export function ownerSelectReaffirm({ endpointId, targetId, targetFamily, expectedOldProofClosureDigest, reaffirmHandle, authorizedBy, chatId, selectedSessionId, selectedRootOm, selectionMessageId, now = undefined, clock = () => Date.now(), env = process.env, _inject } = {}) {
  ...
  return gatedTx({
    ...
    mutate: (doc) => {
      ...
      const remap = Object.values(doc.records)
        .filter((r) => r.kind === "forwarding_tombstone" && r.forwards_to === targetId && r.proof_ref?.kind === "owner_select_merge_v1")
      ...
      const result = ownerSelectBinding
        ? { target_id: targetId, affected_live_ids_after_commit: [targetId], proof_effects: [{ topic_agent_id: targetId, binding_effect: "produced", link_effect: "produced" }], new_binding_proof: { kind: "owner_select_v1", authorized_by: authorizedBy, authorized_at: iso, selected_session_id: rec.aliases.session_id, selected_root_om: rec.aliases.root_om, selection_handle: reaffirmHandle, selection_operation_id: null }, new_link_proof: newLink(), tombstone_remap: remap, selection_message_id: selectionMessageId }
        : { target_id: targetId, affected_live_ids_after_commit: [targetId], proof_effects: [{ topic_agent_id: targetId, binding_effect: "preserved", link_effect: "produced" }], new_link_proof: newLink(), tombstone_remap: remap, selection_message_id: selectionMessageId };
      return { ok: true, next: stampAndBuild(doc, {
        ...
        mutateRecords: (n, opId) => {
          ...
          if (ownerSelectBinding) result.new_binding_proof.selection_operation_id = opId;
          ...
          if (ownerSelectBinding) r.binding_proof = result.new_binding_proof;
          r.updated_at = iso; r.origin_operation_id = opId;
