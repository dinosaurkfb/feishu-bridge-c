...
description: 说明在接了飞书桥的项目里干活时，本地输入与回答会确定性配对后以卡片转发到飞书；飞书来源输入不会重复。当你在这类项目里工作、或想知道进展怎么汇报给 Frank 时使用。你不需要手写进展。也包含把一个新项目接进飞书的一次性命令——当 Frank 明确要求「把这个项目接到飞书」「给这个项目建个话题」时使用。
---
...
接完之后**出站立刻可用**，下一轮本地输入与回答就会合成卡片发到新话题。
