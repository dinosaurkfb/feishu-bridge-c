...
description: M5Claude 专用的受控飞书入站入口。当有人 @M5Claude 时使用 —— 绑定话题、还没接入的群话题、私聊都算（未接入的上下文由脚本按 chat 默认态同步回答；正文前缀已退役，不再要求以特定字符串开头）。执行一条命令，把它的输出原样回复。所有校验、claim、投递都在命令内部以确定性代码完成。不得用于发布进展、创建话题、处理普通回复或触发持续监听。
---

# M5Claude 飞书入站路由

版本：`v2.1 / Aily 命名空间 / 已绑定路径非阻塞投递、秒级回执 / 未接入上下文 chat 默认态（有预算的同步回答，最长 60 秒）`
